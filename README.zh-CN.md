...
5. 在列表中查看捕获变量，或从单条 run 详情把变量对象带入 `JSON Tools`；也可以和源模板进入 `Prompt Diff` 对比、补充维护备注、导出 JSON，或把已保存 prompts 带入新的模板草稿后再编辑保存
...
- 没有后端、账号系统和跨设备同步
- 当前没有跨标签页同步；同时在多个标签页编辑时，后写入的数据可能覆盖先前的本地更改
