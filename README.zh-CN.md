...
| Prompt Workflows | Prompt Run History | 浏览 runs、按模板过滤、搜索备注、查看详情、添加备注、导出单条 run JSON、继续在下游工具查看输出 | 已保存 prompt 输出的历史视图 |
...
5. 打开单条 run 详情，补充维护备注，必要时导出 JSON，再进入 `Prompt Diff` 或 `Code Viewer` 继续检查结果
